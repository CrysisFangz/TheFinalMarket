...
module AdminTransactions
  module Events
    # Domain event representing when an approval step is rejected
    # This event terminates the approval workflow with a rejection
    #
    # @author Kilo Code Autonomous Agent
    # @version 2.0.0
    class ApprovalStepRejectedEvent < DomainEvent
      ...
      # @param step_index [Integer] index of the rejected step
      ...
      # @return [Integer] index of the rejected step
      attr_reader :step_index

      # @return [String] rejection comments
      attr_reader :comments

      # @return [Boolean] true if rejection includes comments
      def has_comments?
        !@comments.nil?
      end
